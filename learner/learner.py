...
class Learner:
    ...
    def __init__(self, session, graph, hamiltonian, machine, sampler, trainer, learning_rate, num_epochs=10000,
                 minibatch_size=0, window_period=200, reference_energy=None, stopping_threshold=0.005,
    ...
    def learn(self):
        ...
        apply_ops = self.trainer(self.learning_rate).apply_gradients(
            [(accum_vars[i] / num_minibatch, tv) for i, tv in enumerate(tf.trainable_variables())])
        ...
        for epoch in range(self.num_epochs):
            ...
            for i in range(num_minibatch):
                ...
                ground_energy_quantile_arr.append(
                    [np.quantile(eloc_np, 0), np.quantile(eloc_np, 0.25), np.quantile(eloc_np, 0.5),
                     np.quantile(eloc_np, 0.75), np.quantile(eloc_np, 1)])
                new_sample_set_np.append(mini_new_sample_np)

            # process energy and error
            ground_energy, ground_energy_std, energy_burn, energy_burn_std, rel_error = self.process_energy_and_error(
                epoch, ground_energy_acc,
            ...
            print('Epoch: %d, energy: %.4f, std: %.4f, std / mean: %.4f, relerror: %.5f, time: %.4f' % (
                epoch, ground_energy, ground_energy_std, ground_energy_std / np.abs(ground_energy), rel_error,
                time_interval))
    ...
    def get_gradient(self, sample_set, sample_size, eloc):
        ...
        for name, derlog in derlogs.items():
            ...
            grad_dict[name] = 2 * tf.matmul(tf.transpose(eloc - eloc_mean),
                                            tf.conj(derlog - derlog_mean)) / self.minibatch_size
    ...
    def process_energy_and_error(self, epoch, ground_energy_acc, ground_energy_std_arr, ground_energy_skew_arr,
                                 ground_energy_kurtosis_arr, ground_energy_quantile_arr):
