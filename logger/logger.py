...
class Logger(object):
    ...
    def __init__(self, log=True, result_path=default_result_path, subpath=default_subpath, visualize_weight=False,
                 visualize_visible=False, visualize_freq=10, observables=None, observable_array=None, weight_diff=False):
        self.log_value = log
    ...
    def calculate_observables(self, learner, num_samples=10000, num_steps=30, num_division=1):
        ...
        if len(learner.sample_set) == 0:
            ...
            for i in range(num_division):
                ...
                samples = learner.session.run(
                    learner.sampler.sample(learner.machine, init_samples, num_samples / num_division))
                if params.size == 0:
                    params = samples
                else:
    ...
    def write_logs(self, learner):
        ...
        ff = open(path, 'w')
        ff.write(
            'epoch,ground_energy,ground_energy_std,ground_energy_skew,ground_energy_kurtosis,ground_energy_window,rel_error,time\n')
        for ep, (ge, ges, gesk, gek, gew, gest, re, ti) in enumerate(
                zip(ground_energys, ground_energy_stds, ground_energy_skews, ground_energy_kurtosises, energy_burns,
                    energy_burns_std, rel_errors, times)):
        ...
        ff.write('%d,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f\n' % (
            self.num_experiment, energy_burns[-1], energy_burns_std[-1], ground_energy_stds[-1],
    ...
    def visualize_params(self, learner):
        ...
        self.histo_weights_rbm(learner)
        self.visualize_weights_rbm_alt(learner)
    ...
    def visualize_weights_rbm_scat(self, learner):
        path = self.result_path + '/weights/'
        if not os.path.exists(path):
            os.makedirs(path)
    ...
    def histo_weights_rbm(self, learner):
        ...
        for epoch, title, w in learner.rbm_weights:
            self.histo(w.real, path, epoch, title, learner)
    ...
    def relative_errors(self, learner):
        plt.figure()
        plt.title('Relative Error vs Iteration, %s, %s' % (str(learner.hamiltonian), str(learner.machine)))
        plt.ylabel('Relative Error')
        plt.xlabel('Iteration #')
        rel_errors = np.array(learner.rel_errors)
        plt.plot(range(len(rel_errors)), rel_errors, label='Relative Error')
        plt.legend(frameon=False)
        plt.tight_layout()
        plt.show()
        plt.savefig(self.result_path + '/iter-error.png')
        plt.close()

    def iter_burns(self, learner):
        plt.figure()
        plt.title('Energy Burns vs Iteration, %s, %s' % (str(learner.hamiltonian), str(learner.machine)))
        plt.ylabel('Energy')
        plt.xlabel('Iteration #')
        energy_burns = np.array(learner.energy_burns)
        energy_burns_std = np.array(learner.energy_burns_std)
        plt.plot(range(len(energy_burns)), energy_burns, label='energy')
        plt.fill_between(range(len(energy_burns)), energy_burns - energy_burns_std,
                         energy_burns + energy_burns_std, alpha=0.4, color='red')
        plt.legend(frameon=False)
        plt.tight_layout()
        plt.show()
        plt.savefig(self.result_path + '/iter-burns.png')
        plt.close()

    def ge_skew(self, learner):
        plt.figure()
        plt.title('Ground Energy Skew vs Iteration, %s, %s' % (str(learner.hamiltonian), str(learner.machine)))
        plt.ylabel('Ground Energy Skew')
        plt.xlabel('Iteration #')
        ground_energy_skew = np.array(learner.ground_energy_skew)
        plt.plot(range(len(ground_energy_skew)), ground_energy_skew, label='Ground Energy Skew')
        plt.legend(frameon=False)
        plt.tight_layout()
        plt.show()
        plt.savefig(self.result_path + '/ground_energy_skew-iter.png')
        plt.close()

    def ge_kurtosis(self, learner):
        plt.figure()
        plt.title('Ground Energy Kurtosis vs Iteration, %s, %s' % (str(learner.hamiltonian), str(learner.machine)))
        plt.ylabel('Ground Energy Kurtosis')
        plt.xlabel('Iteration #')
        ground_energy_kurtosis = np.array(learner.ground_energy_kurtosis)
        plt.plot(range(len(ground_energy_kurtosis)), ground_energy_kurtosis, label='Ground Energy Kurtosis')
        plt.legend(frameon=False)
        plt.tight_layout()
        plt.show()
